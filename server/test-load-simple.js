...
// Мокаем базу данных для тестирования производительности
const mockDatabase = {
  find: async () => {
    // Имитация быстрого ответа из кэша
    await new Promise(resolve => setTimeout(resolve, 1));
    return [];
  },
  save: async () => {
    // Имитация быстрой записи в базу данных
    await new Promise(resolve => setTimeout(resolve, 5));
    return { success: true, saved: 1, updated: 0 };
  }
};

// Переопределяем методы работы с базой данных в SourcesFetcher
SourcesFetcher.prototype.findSources = async function(animeId, episodeNumber) {
  return mockDatabase.find();
};

SourcesFetcher.prototype.saveEpisodeSources = async function(sources, animeId) {
  return mockDatabase.save();
};

async function runSimpleLoadTests() {
  try {
    console.log('🚀 Запуск упрощенного нагрузочного тестирования...');
    
    // Тест 1: Последовательные запросы
    console.log('\n🔍 Тест 1: Последовательные запросы (30 запросов)');
    ...
    console.log(`   - Запросов в секунду: ${(30 / ((sequentialEnd - sequentialStart) / 1000)).toFixed(2)}`);
    
    // Тест 2: Параллельные запросы
    console.log('\n🔍 Тест 2: Параллельные запросы (15 одновременных запросов)');
    ...
    console.log('\n🔍 Тест 5: Бurst нагрузка (10 запросов за короткий период)');
    ...
    console.log(`   - Среднее время на запрос: ${(burstEnd - burstStart) / 10}ms`);
    
    console.log('\n✅ Упрощенное нагрузочное тестирование завершено успешно');
