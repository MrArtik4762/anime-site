import Joi from 'joi';
import { HTTP_STATUS, LIMITS, REGEX } from '../../shared/constants/constants.js';
  ...
  animeEpisode: Joi.object({
    id: Joi.string()
      ...
      .messages({
        ...
        'any.required': 'ID аниме обязательно'
      }),
...
const querySchemas = {
  ...
  animeSearch: Joi.object({
    ...
    year: Joi.alternatives()
      ...
      .messages({
        ...
        'number.max': 'Год не может быть слишком далеким в будущем'
      }),
  ...
  // Схема валидации для стриминга
  stream: Joi.object({
    url: Joi.string()
      .uri()
      .required()
      .messages({
        'string.uri': 'URL должен быть валидным URI',
        'any.required': 'URL обязателен для стриминга'
      })
  })
};

// Схемы валидации для отслеживания прогресса просмотра
const watchProgressSchemas = {
  saveProgress: Joi.object({
    ...
    position: Joi.number()
      ...
      .required()
      .messages({
        'number.min': 'Позиция видео не может быть отрицательной',
        'any.required': 'Позиция видео обязательна'
      }),
    ...
    duration: Joi.number()
      ...
      .required()
      .messages({
        'number.min': 'Длительность видео должна быть больше 0',
        'any.required': 'Длительность видео обязательна'
      })
  }),
