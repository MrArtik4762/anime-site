...
const EnhancedAniLibertyPlayer = ({
  ...
  const handleVoiceChange = useCallback((voice) => {
    if (voice) {
      ...
      // Обновляем настройки
      if (settings) {
        saveVideoSettingsDB({ ...settings, voice: voice.id });
      }
      
      // Если у озвучки есть URL, перезагружаем видео
      if (voice.url && videoRef.current) {
        const currentTime = videoRef.current.currentTime;
        const wasPlaying = !videoRef.current.paused;
        
        // Обновляем источник видео
        if (hlsRef.current) {
          hlsRef.current.loadSource(voice.url);
        } else {
          videoRef.current.src = voice.url;
        }
      ...
      <ControlsOverlay visible={controlsVisible && !isLoading && !error}>
        ...
        <ControlsBar>
          ...
          <RightControls>
            ...
            <VoiceSelector
              episodeId={episodeId}
              selectedVoice={availableVoices[currentVoice]}
